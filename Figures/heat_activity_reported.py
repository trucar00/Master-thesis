...
# ------------------------------------------------------------
files = [
    "heatmap_of_activity/2024_1_3.parquet",
    "heatmap_of_activity/2024_4_6.parquet",
    "heatmap_of_activity/2024_7_9.parquet",
    "heatmap_of_activity/2024_10_12.parquet",
]
...
GEARS = ["Not", "Krokredskap", "Bur og ruser", "Garn", "Snurrevad", "Trål"]
...
for f in files:
    table = pq.read_table(
        f,
        columns=["mmsi", "date_time_utc", "lon", "lat", "report"],
        filters=[("report", "in", GEARS)]
    )

    df_part = table.to_pandas()
    df_part["pred_fishing"] = df_part["report"].isin(GEARS).astype(int)
    dfs.append(df_part)
...
# ------------------------------------------------------------
output_path = Path("heatmap_of_activity/fishing_effort_grid_hours_reported_2024.parquet")

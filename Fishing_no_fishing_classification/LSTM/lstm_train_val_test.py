...
print(f"Train (2023) vessels: {len(train_mmsis)} | Val (2024) vessels: {len(val_mmsis)} | Test (2024) vessels: {len(test_mmsis)}")


# Normalization stats -- fit on TRAIN (2023) only

mu_sigma_path = Path(f"{FOLDER}/mu_sigma_{TAG}.pkl")
if mu_sigma_path.exists():
    ...
    print(f"Fit mu/sigma on training mmsis in 2023 and saved to {mu_sigma_path}")
...
# Normalize the test sets using the mu and sigma from the training mmsis in 2023.
...
# TEST on future UNSEEN vessels -> foreign vessels 
...
def predict_and_score_test(model, seen, seed):
    if seen:
        df = df_test_seen
        prefix = "seen"
    else:
...
for seed in SEEDS:
    ...
    model_name = f"{FOLDER}/Models/model_lstm_seed{seed}_test.pt"
    best_val = float("inf")
    bad = 0
    history = []
    ...
    test_unseen = predict_and_score_test(model, seen=False, seed=seed)
